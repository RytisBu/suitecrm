...
class AdministrationController extends SugarController
{
    ...
    public function action_testRole(){
        global $current_user, $db, $role;

        if(!empty($_SESSION['spectater_id'])){

            $user = new User();
            $user->retrieve($_SESSION['authenticated_user_id']);
            if($db->query("SELECT role_id FROM acl_roles_users WHERE user_id='{$user->id}'"))
            $role = new ACLRole();
            $role->retrieve($_SESSION['authenticated_user_id']);
            dump($role);
            exit;
    ...
    public function action_getRolesList(){
        global $current_user;
        if($current_user->isAdmin() && !empty($_SESSION['spectater_id'])){
            dump($_SESSION['spectater_id']);
            dump($_SESSION['authenticated_user_id']);
            exit;
        }else{
            dump($_SESSION['spectater_id']);
            dump($_SESSION['authenticated_user_id']);
    ...
    public function action_spectate(){
        global $current_user, $GLOBALS, $user, $db, $timestamp;
        $user = new User();
        $user->retrieve($current_user->id);
        if($current_user->isAdmin()){

            $_SESSION['spectater_id'] = $current_user->id;
            $_SESSION['authenticated_user_id'] = $_REQUEST['user_id'];

            $_SESSION['time_start'] = date('Y-m-d H:i:s');
            dump($_SESSION['time_start']);
            $db->query("INSERT INTO spectate_info (admin_id, spectated_user_id, spectate_time_start) VALUE ('{$_SESSION['spectater_id']}', '{$_SESSION['authenticated_user_id']}', NOW())");
            $GLOBALS['log']->fatal("Spectating started. User {$current_user->user_name} spectates {$_REQUEST['user_id']} user id");
            SugarApplication::redirect("index.php?module=Home&action=index");
        }
    }

    public function action_end_spectate(){
        global $db;
        if(!empty($_SESSION['spectater_id'])){
            $_SESSION['authenticated_user_id'] = $_SESSION['spectater_id'];
            $db->query("UPDATE spectate_info SET spectate_time_end = NOW() WHERE admin_id={$_SESSION['spectater_id']} and spectate_time_start = '{$_SESSION['time_start']}'");
            unset($_SESSION['spectater_id']);
